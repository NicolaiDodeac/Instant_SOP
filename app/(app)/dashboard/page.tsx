...
export default function DashboardPage() {
  ...
  const [sopAuthors, setSopAuthors] = useState<Record<string, SopAuthorInfo>>({})
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!res.ok) return
        const data = (await res.json()) as { authors?: Record<string, SopAuthorInfo> }
        if (!cancelled) setSopAuthors(data.authors ?? {})
      } catch {
        if (!cancelled) setSopAuthors({})
      }
    })()
    return () => {
      cancelled = true
                    ...
                    <div className="flex items-stretch justify-between gap-2">
                      ...
                      <div className="flex flex-col items-end self-stretch shrink-0 gap-1">
                        ...
                        <div className="flex min-h-0 flex-1 flex-col items-end justify-end">
                          {sopAuthors[sop.id] ? (
                            <SopAuthorAvatar author={sopAuthors[sop.id]!} size={32} />
                          ) : null}
                        </div>
                      </div>
                    </div>
