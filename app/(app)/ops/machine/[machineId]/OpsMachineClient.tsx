...
import type { MachineFamilyStation, OpsContextSopsPayload, OpsMachineContext, SOP } from '@/lib/types'
...
type ScopeResults = OpsContextSopsPayload['results']
...
export default function OpsMachineClient({
  machineId,
  initialCtx,
  initialSops = null,
  initialSopsFetchKey = null,
}: {
  machineId: string
  initialCtx: OpsMachineContext
  initialSops?: OpsContextSopsPayload | null
  initialSopsFetchKey?: string | null
}) {
  ...
  const [sops, setSops] = useState<OpsContextSopsPayload | null>(() => initialSops ?? null)
  ...
  useEffect(() => {
    const fetchKey = buildContextSopsFetchKey(
      ctx.machine.id,
      stationCode,
      stationIdParam,
    ...
    void (async () => {
      ...
      try {
        ...
        const body = (await res.json()) as OpsContextSopsPayload
        setSops(body)
      } finally {
